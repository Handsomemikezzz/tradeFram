...
import { DashboardOverviewResponse } from '@/services/api';

interface TodayTasksProps {
  tasks?: DashboardOverviewResponse['tasks'];
}

export const TodayTasks = ({ tasks }: TodayTasksProps) => {
  const taskItems = [
    { title: '风控拦截待查看', count: tasks?.riskBlockedToReview ?? 0, color: (tasks?.riskBlockedToReview ?? 0) > 0 ? 'text-red-600' : 'text-gray-400' },
    { title: '数据超过 24 小时未更新', count: tasks?.staleDataOver24h ?? 0, color: 'text-gray-400' },
    { title: '研究报告生成失败', count: tasks?.failedResearchReports ?? 0, color: (tasks?.failedResearchReports ?? 0) > 0 ? 'text-red-600' : 'text-gray-400' },
    { title: '交易监控池暂停股票', count: tasks?.pausedMonitoringStocks ?? 0, color: 'text-gray-500' },
  ];
